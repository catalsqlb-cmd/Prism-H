# Prism Trae Adaptation Guide (Workflow Runbook)

Use Prism research workflows in Trae without relying on Claude Code `/skill-name` slash commands.
...
| Skill storage | `~/.claude/skills/...` | Global `~/.trae/skills/` (cross-project available) or project `<project>/.trae/skills/` (current project only), or directly reference Prism repo `skills/` |
...
It is recommended to create a dedicated Trae agent for Prism workflows to avoid conflicts with other agents and to keep role instructions stable.
...
3. Click "Import File" and select SKILL.md files from the Prism repo's `skills/` directory
...
Copy-Item -Path "C:\path\to\Prism\skills\*" -Destination "$env:USERPROFILE\.trae\skills\" -Recurse -Force

# Project-level installation (available only in current project)
New-Item -ItemType Directory -Path ".\.trae\skills" -Force
Copy-Item -Path "C:\path\to\Prism\skills\*" -Destination ".\.trae\skills\" -Recurse -Force
```
...
Prism relies on an executor model + external reviewer model. Configure reviewer MCP first, then run workflows.
...
```json
{
  "mcpServers": {
    "llm-chat": {
      "command": "/path/to/Prism/.venv/Scripts/python.exe",
...
Note: `@skills/.../SKILL.md` references only resolve if the Prism repo (or its `skills/` folder) is part of the current Trae workspace. They will not work when the skills folder exists only in a separate workspace.
...
Trae automatically discovers Prism skills via the YAML `description` field in `SKILL.md`. Below are invocation methods for each workflow:
...
| Prism MCP tool | Purpose | Required MCP server |
|---|---|---|
...
- [ ] Import Prism skills' SKILL.md files
