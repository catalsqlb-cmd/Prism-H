...
本文档说明如何使用阿里百炼 Coding Plan 的一套 API Key 驱动 Prism 全流程，无需 Claude 或 OpenAI API。
...
原版 Prism 的审查器使用 Codex MCP，而 Codex CLI 硬编码调用 OpenAI 专有的 **Responses API** (`/v1/responses`)，第三方 API 提供商（包括 Coding Plan）均不支持该接口。
...
Coding Plan 提供两套 API 端点，分别对应 Prism 的两个角色：
...
> **关于跨模型优势**：Prism 的核心设计是执行器和审查器使用不同模型，互不审自己的输出，形成真正的反馈循环。如果两端用同一模型（如都用 kimi-k2.5），会部分削弱这一优势。
