...
> How to maintain project state across sessions and context compaction in Prism workflows — the core design is **Pipeline Status in your project CLAUDE.md**, with optional Claude Code hooks for automation.

## Why Session Recovery Matters

Prism workflows can run for hours (idea discovery, auto-review loops, overnight training). Two things regularly break state continuity:
...
Prism already persists some state to files (`review-stage/REVIEW_STATE.json`, `review-stage/AUTO_REVIEW.md`), but **there is no systematic mechanism to ensure the LLM reads those files on recovery**. After compaction, it often doesn't.
...
RESEARCH_ROOT="${PRISM_RESEARCH_ROOT:-$HOME/research}"
CWD=$(pwd)

[[ "$CWD" != "$RESEARCH_ROOT"/* ]] && exit 0

# Once per session (PID-based flag)
FLAG="/tmp/prism-session-restore-$$"
[ -f "$FLAG" ] && exit 0
touch "$FLAG"
...
COUNTER_FILE="/tmp/prism-context-refresh-counter"
COUNT=0
...
COUNTER_FILE="/tmp/prism-progress-remind-counter"
COUNT=0
[ -f "$COUNTER_FILE" ] && COUNT=$(cat "$COUNTER_FILE" 2>/dev/null || echo 0)
COUNT=$((COUNT + 1))
echo "$COUNT" > "$COUNTER_FILE"
...
```bash
export PRISM_RESEARCH_ROOT="$HOME/my-projects"  # default: ~/research
```
