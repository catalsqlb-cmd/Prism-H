...
> 在 Prism 工作流中跨会话和上下文压缩维护项目状态 — 核心设计是**项目 CLAUDE.md 中的 Pipeline Status**，Claude Code 用户可选 hook 自动化。

## 为什么需要会话恢复

Prism 工作流可能持续数小时（idea discovery、auto-review loop、overnight training）。两件事会打断状态连续性：
...
Prism 已经将部分状态持久化到文件（`review-stage/REVIEW_STATE.json`、`review-stage/AUTO_REVIEW.md`），但**没有系统性机制确保 LLM 在恢复时去读这些文件**。压缩后，它经常忘记。
...
```bash
export PRISM_RESEARCH_ROOT="$HOME/my-projects"  # 默认: ~/research
```
