# Antigravity Adaptation Guide (Prism Workflows)

> Use Prism research workflows in **Google Antigravity** — the agent-first AI IDE from Google DeepMind.

Antigravity natively supports `SKILL.md` files with the same YAML frontmatter + Markdown body format used by Prism, making it one of the most natural hosts for Prism workflows.
...
Antigravity supports multiple models as the **executor** (the model that runs Prism workflows):
...
- Prism skill instructions will be followed very faithfully
...
```bash
git clone https://github.com/catalsqlb-cmd/Prism.git
cd Prism
...
Prism uses an external LLM (GPT-5.4 via Codex) as a critical reviewer. To enable this in Antigravity:
...
2. Add MCP server — edit `~/.gemini/settings.json`. Both paths must be **absolute**:
   ```json
   {
     "mcpServers": {
       "llm-chat": {
         "command": "/path/to/Prism/.venv/bin/python3",
         "args": ["/path/to/Prism/mcp-servers/llm-chat/server.py"],
         "env": {
...
Antigravity discovers Prism skills via the YAML `description` field in each `SKILL.md`. There are three approaches:
...
If Prism skills are installed (§3.1), Antigravity will automatically discover and activate the `auto-review-loop` skill.
...
Prism skills reference MCP tools by name. These work identically in Antigravity once configured:

| Prism MCP tool | What it does | Required MCP server |
|--------------|-------------|-------------------|
...
Prism workflows persist state to files for crash recovery. These work identically in Antigravity:
...
Antigravity provides several unique capabilities that enhance Prism workflows:

### Multi-Agent Orchestration
Use Antigravity's **Manager View** to run multiple Prism stages simultaneously:
...
### Artifact System
Antigravity's artifact system (implementation plans, walkthroughs) maps naturally to Prism outputs:
