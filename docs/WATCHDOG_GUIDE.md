...
> Persistent server-side monitoring for Prism experiments — catch dead sessions, stalled downloads, and idle GPUs without manual polling.

## The Problem

Prism experiments run remotely in screen/tmux sessions. Current monitoring (`/monitor-experiment`) is **on-demand** — you have to actively ask "how are my experiments doing?". Between checks:
...
# or
ssh your-server "cat /tmp/prism-watchdog/status/summary.txt"
```
...
```bash
ssh your-server "tail -20 /tmp/prism-watchdog/alerts.log"
```
...
|-----------|---------|---------------|
| Base directory | `/tmp/prism-watchdog` | `--base-dir /your/path` |
