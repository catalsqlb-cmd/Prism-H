...
- This path is designed to maximize reuse of the original Prism reviewer-aware skills while minimizing skill changes.
...
- Using the direct Gemini API removes the extra local CLI hop and keeps the reviewer path closer to the API-backed integrations already used elsewhere in Prism.
...
This does not change the Prism reviewer contract or skill overlay shape. It only changes the Gemini API model used behind the same local `gemini-review` bridge.
