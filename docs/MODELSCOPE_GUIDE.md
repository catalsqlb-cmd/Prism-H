...
本文档说明如何使用 [ModelScope](https://www.modelscope.cn/) 的免费 API 驱动 Prism 全流程，无需 Claude 或 OpenAI API。
...
原版 Prism 的审查器使用 Codex MCP，而 Codex CLI 硬编码调用 OpenAI 专有的 **Responses API** (`/v1/responses`)，第三方 API 提供商（包括 ModelScope）均不支持该接口。
...
ModelScope 同时提供两套 API 端点，分别对应 Prism 的两个角色：
...
> **关于跨模型优势**：Prism 的核心设计是执行器和审查器使用不同模型，形成真正的反馈循环。推荐使用不同模型族（如 DeepSeek 执行 + Qwen 审查）以最大化对抗性。
...
**ModelScope 的独特优势**：免费 + 无自动化限制 + 一个 Key，是最低门槛的 Prism 接入方式。
