...
#     bash tools/smart_update.sh --project <path> --target-subdir .agents/skills/prism [--apply]
#   Custom paths:
...
#                               common values: .claude/skills, .claude/skills/prism,
#                                              .agents/skills, .agents/skills/prism
...
# ─── Deprecate nested --target-subdir (.claude/skills/prism, .agents/skills/prism) ──
# Nested install hides skills from Claude Code's slash-command discovery (it scans
# only one level deep). The replacement is the flat install via install_prism.sh.
if [[ "$TARGET_SUBDIR" == ".claude/skills/prism" || "$TARGET_SUBDIR" == ".agents/skills/prism" ]]; then
    REPO_ROOT_FOR_HINT="$(cd "$(dirname "$0")/.." && pwd)"
    echo "" >&2
    echo -e "\033[1;33m⚠️  --target-subdir $TARGET_SUBDIR is DEPRECATED\033[0m" >&2
    echo "" >&2
    echo "  Reason: the nested 'prism/' subdirectory hides skills from Claude Code's" >&2
    ...
    echo "    bash $REPO_ROOT_FOR_HINT/tools/install_prism.sh \"${PROJECT_PATH:-<project>}\"" >&2
    echo "" >&2
    echo "  To migrate an existing nested install:" >&2
    echo "    bash $REPO_ROOT_FOR_HINT/tools/install_prism.sh \"${PROJECT_PATH:-<project>}\" --from-old" >&2
    echo "    (for COPY-style installs, also pass --migrate-copy keep-user|prefer-upstream)" >&2
    echo "" >&2
    if $APPLY; then
        echo -e "\033[0;31mRefusing to --apply with deprecated nested target. Use install_prism.sh instead.\033[0m" >&2
        exit 2
    fi
...
# ─── Refuse to operate on symlinked installs (those use install_prism.sh, not smart_update) ──
if [[ -L "$LOCAL_DIR" ]]; then
    ...
    echo "  → $LINK_TARGET" >&2
    echo "" >&2
    echo "smart_update is for COPIED installs. Symlinked installs are managed by install_prism.sh:" >&2
    echo "  cd <prism-repo> && git pull           # updates content of existing skills" >&2
    echo "  bash $REPO_ROOT_FOR_HINT/tools/install_prism.sh <project>   # reconciles new/removed skills" >&2
    echo "" >&2
    exit 2
fi
...
echo -e "${BLUE}━━━ Prism Smart Skill Update ━━━${NC}"
echo -e "Scope:    ${SCOPE}"
