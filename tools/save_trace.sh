#!/usr/bin/env bash
# save_trace.sh — Save a reviewer MCP call trace to .prism/traces/
# Part of the Prism Review Tracing Protocol (shared-references/review-tracing.md)
#
# Usage:
...
PROMPT_FILE="" RESPONSE_FILE="" TRACE_MODE="${PRISM_TRACE_MODE:-full}"
...
EVENTS_FILE=".prism/meta/events.jsonl"
if [[ -d ".prism/meta" ]]; then
  python3 -c "
import json
evt = {
    'event': 'review_trace',
