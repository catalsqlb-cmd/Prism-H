#!/usr/bin/env bash
# verify_paper_audits.sh — External verifier for Prism mandatory paper audits.
#
...
#
# Defaults:
#   --assurance: read from <paper-dir>/.prism/assurance.txt if present, else "draft"
#   --json-out:  <paper-dir>/.prism/audit-verifier-report.json
#
# Exit codes:
...
if [[ -z "$ASSURANCE" ]]; then
    if [[ -f "$PAPER_DIR/.prism/assurance.txt" ]]; then
        ASSURANCE="$(tr -d '[:space:]' < "$PAPER_DIR/.prism/assurance.txt")"
    else
        ASSURANCE="draft"
    fi
fi
...
[[ -n "$JSON_OUT" ]] || JSON_OUT="$PAPER_DIR/.prism/audit-verifier-report.json"
