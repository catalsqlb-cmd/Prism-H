...
# Prism Smart Skill Update (PowerShell)
...
#     .\tools\smart_update.ps1 -ProjectPath <path> -TargetSubdir '.agents/skills/prism' [-Apply]
#   Custom paths:
...
#                  common: .claude/skills, .claude/skills/prism, .agents/skills, .agents/skills/prism
...
if ($PSCmdlet.ParameterSetName -eq 'Project') {
    ...
    # ─── Deprecate nested -TargetSubdir (.claude/skills/prism, .agents/skills/prism) ──
    if ($TargetSubdir -in @('.claude/skills/prism', '.agents/skills/prism')) {
        Write-Host ""
        Write-Host "⚠️  -TargetSubdir $TargetSubdir is DEPRECATED" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "  Reason: nested 'prism/' subdirectory hides skills from Claude Code's slash-command discovery" -ForegroundColor Yellow
        ...
        Write-Host "  see install_prism.ps1 docstring for the manual one-liner)." -ForegroundColor Yellow
        Write-Host ""
        if ($Apply) {
...
if (Test-Path $LocalDir) {
    ...
    if ($item -and ($item.LinkType -in @('Junction', 'SymbolicLink'))) {
        ...
        Write-Host "  cd <prism-repo>; git pull"
        Write-Host ""
...
# ─── Header ────────────────────────────────────────────────────────────────────
Write-Host ''
Write-Host '=== Prism Smart Skill Update ===' -ForegroundColor Cyan
