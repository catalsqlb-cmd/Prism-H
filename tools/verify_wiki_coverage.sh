...
#
# This is a best-effort diagnostic. Because Prism has no canonical "papers
...
#
# Defaults:
#   --scan: scans CWD-level `.prism/traces/`, `paper/`, `PAPER_PLAN.md`,
...
if [[ ${#SCAN_PATHS[@]} -eq 0 ]]; then
    for p in .prism/traces paper PAPER_PLAN.md NARRATIVE_REPORT.md references.bib; do
        [[ -e "$p" ]] && SCAN_PATHS+=("$p")
    done
fi
