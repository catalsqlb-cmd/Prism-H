#!/usr/bin/env bash
# install_prism.sh — Project-local Prism skill installation (flat per-skill symlinks).
#
# Each Prism skill is symlinked into `<project>/.claude/skills/<skill-name>` so
# Claude Code's slash-command discovery (which only scans one level deep) finds it.
# A versioned manifest at `<project>/.prism/installed-skills.txt` tracks every
...
#
# Usage:
#   bash tools/install_prism.sh [project_path] [options]
#
...
#
# Options:
#   --prism-repo PATH       override prism-repo discovery
...
#   --with-wiki PATH       after install, scaffold a personal LLM Wiki at PATH
#                          (Karpathy three-layer KB; idempotent; never edits raw/)
#
# Safety rules enforced:
#   S1  Never delete a path that is not a symlink.
#   S2  Never delete a symlink whose target is outside the configured prism-repo.
...
#   S9  If .prism/, .claude/, or .claude/skills/ is itself a symlink, abort.
#   S10 Reject upstream entries that are symlinks to outside prism-repo.
...
WITH_WIKI=""         # "" | path to scaffold a personal LLM Wiki after install
ADOPT_NAMES=()
REPLACE_LINK_NAMES=()
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --prism-repo)         PRISM_REPO_OVERRIDE="${2:?--prism-repo requires path}"; shift 2 ;;
        ...
        --with-wiki)         WITH_WIKI="${2:?--with-wiki requires PATH}"; shift 2 ;;
        ...
        --platform)
            echo "Error: --platform is removed. Prism now only supports Claude Code (.claude/skills/)." >&2
...
# Find prism-repo location
resolve_prism_repo() {
    local p
    if [[ -n "$PRISM_REPO_OVERRIDE" ]]; then
        p="$(abs_path "$PRISM_REPO_OVERRIDE")" || die "--prism-repo path not found: $PRISM_REPO_OVERRIDE"
        [[ -d "$p/skills" ]] || die "--prism-repo has no skills/ subdir: $p"
        echo "$p"; return
    fi
    ...
    if [[ -n "${PRISM_REPO:-}" && -d "$PRISM_REPO/skills" ]]; then abs_path "$PRISM_REPO"; return; fi
    for guess in \
        "$HOME/Desktop/prism_repo" \
        "$HOME/prism_repo" \
        "$HOME/.prism" \
        "$HOME/Desktop/Prism" \
        "$HOME/.codex/Prism" \
        "$HOME/.claude/Prism" ; do
        [[ -d "$guess/skills" ]] && { abs_path "$guess"; return; }
    done
    die "cannot find Prism repo. Use --prism-repo PATH or set PRISM_REPO env var."
}

# Build the upstream inventory: array of "kind|name" entries
# Skills = top-level dirs in <prism-repo>/skills/ containing SKILL.md
# Support = explicitly listed support directories (shared-references)
# Rejects: anything in EXCLUDE_TOP_NAMES, names failing slug regex, symlinks to outside prism-repo (S10)
...
# ─── Resolve project path & prism-repo ─────────────────────────────────────────
PROJECT_PATH="${PROJECT_PATH:-$(pwd)}"
[[ -d "$PROJECT_PATH" ]] || die "project path does not exist: $PROJECT_PATH"
PROJECT_PATH="$(abs_path "$PROJECT_PATH")"
PRISM_REPO="$(resolve_prism_repo)"
SKILLS_DIR_ABS="$PRISM_REPO/skills"
PROJECT_SKILLS_DIR="$PROJECT_PATH/$SKILLS_REL"
PROJECT_PRISM_DIR="$PROJECT_PATH/$PRISM_DIR_NAME"
MANIFEST_PATH="$PROJECT_PRISM_DIR/$MANIFEST_NAME"
MANIFEST_PREV="$PROJECT_PRISM_DIR/$MANIFEST_PREV_NAME"
LOCK_DIR="$PROJECT_PRISM_DIR/$LOCK_DIR_NAME"
DOC_FILE="$PROJECT_PATH/$DOC_FILE_NAME"

# ─── S9: refuse if .prism / .claude / .claude/skills is itself a symlink ───────
# (.prism and .claude/skills may not exist yet — only check if present.)
check_no_symlinked_parents() {
    local p
    for p in "$PROJECT_PRISM_DIR" "$PROJECT_PATH/.claude" "$PROJECT_SKILLS_DIR"; do
...
{"host":"$(hostname)","pid":$$,"started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","tool":"install_prism.sh"}
EOF
...
acquire_lock() {
    ...
    local owner=""
    [[ -f "$LOCK_DIR/owner.json" ]] && owner="$(cat "$LOCK_DIR/owner.json")"
    die "another install_prism.sh is running in this project (lock: $LOCK_DIR)
       owner: $owner
...
# ─── Migration detection ──────────────────────────────────────────────────────
LEGACY_NESTED="$PROJECT_SKILLS_DIR/prism"
...
migrate_legacy() {
    ...
    case "$kind" in
       ...
       This may contain user edits. Choose explicitly:
         ...
         --migrate-copy prefer-upstream  (archive nested copy to .prism/legacy-copy-backup-<ts>/
...
archive_legacy_copy() {
    ...
    local archive="$PROJECT_PRISM_DIR/legacy-copy-backup-$ts"
    ...
    while IFS='|' read -r action kind name extra; do
        ...
        case "$action" in
            ...
            UPDATE_TARGET)
                ...
                # S2: stale target must point inside prism-repo
                if [[ "$plan_saw_target" != "$PRISM_REPO"/* ]]; then
                    warn "S2: refusing to replace symlink pointing outside prism-repo: $target_path -> $plan_saw_target"
                    continue
                fi
            ...
            REMOVE)
                ...
                # S2: target must be inside prism-repo
                local cur; cur="$(read_link_target "$target_path")"
                [[ "$cur" != /* ]] && cur="$(canonicalize "$(dirname "$target_path")/$cur")"
                [[ "$cur" == "$PRISM_REPO"/* ]] || { warn "S2: $target_path target $cur outside prism-repo, refusing"; continue; }
...
## Prism Skill Scope
Prism skills installed in this project: $count entries.
Manifest: \`$PRISM_DIR_NAME/$MANIFEST_NAME\` (lists every skill Prism installed and its upstream target).
For Prism workflows, prefer the project-local skills under \`$SKILLS_REL/\` over global skills.
Do not modify or delete files inside any skill that is a symlink (symlinks point into \`$PRISM_REPO\`).
Update with: \`bash $PRISM_REPO/tools/install_prism.sh\`  (re-runnable; reconciles new/removed skills).
$BLOCK_END"
...
if len(matches) > 1:
    sys.stderr.write("PRISM:WARN multiple Prism blocks found in CLAUDE.md; skipping update\n")
...
PYEOF
    ...
    if $DRY_RUN; then log "  (dry-run) would update CLAUDE.md Prism block"; return 0; fi
    tmp="$DOC_FILE.prism-tmp.$$"
    ...
    mv -f "$tmp" "$DOC_FILE"
    log "  ✓ updated CLAUDE.md (Prism managed block)"
}

# ─── Uninstall ────────────────────────────────────────────────────────────────
do_uninstall() {
    [[ -f "$MANIFEST_PATH" ]] || die "no manifest at $MANIFEST_PATH; nothing to uninstall"
    local manifest_data; manifest_data="$(mktemp -t prism-manifest.XXXX)"
...
# ─── Main flow ────────────────────────────────────────────────────────────────
log ""
log "Prism Project Install"
log "  Project:    $PROJECT_PATH"
log "  Prism repo:  $PRISM_REPO"
...
# Build inventories
UPSTREAM_FILE="$(mktemp -t prism-upstream.XXXX)"
build_upstream_inventory "$PRISM_REPO" > "$UPSTREAM_FILE"
[[ -s "$UPSTREAM_FILE" ]] || die "upstream inventory empty (broken prism-repo?)"

MANIFEST_DATA="$(mktemp -t prism-manifest.XXXX)"
load_manifest "$MANIFEST_PATH" "$MANIFEST_DATA"

PLAN_FILE="$(mktemp -t prism-plan.XXXX)"
...
if $DRY_RUN; then
    log ""
    [[ -n "$WITH_WIKI" ]] && log "(dry-run) would scaffold LLM Wiki at: $WITH_WIKI"
...
INSTALLED_NAMES="$(mktemp -t prism-names.XXXX)"
...
# Optional: scaffold a personal LLM Wiki
if [[ -n "$WITH_WIKI" ]]; then
    WIKI_TOOL="$PRISM_REPO/tools/llm_wiki.py"
    if [[ ! -f "$WIKI_TOOL" ]]; then
        warn "--with-wiki: $WIKI_TOOL not found; skipping wiki scaffold"
    else
        log ""
        log "Scaffolding personal LLM Wiki at: $WITH_WIKI"
        if python3 "$WIKI_TOOL" init "$WITH_WIKI"; then
            log "✓ LLM Wiki ready. Drop sources into raw/, then run /llm-wiki ingest."
        else
            warn "--with-wiki: llm_wiki.py init exited non-zero"
        fi
    fi
fi
