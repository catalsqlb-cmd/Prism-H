...
Bridge Codex-first Prism workflows to Gemini, using the direct Gemini API by default.
...
- The bridge defaults to the direct Gemini API path. This is the intended reviewer backend for the Prism skill overlay.
...
- This is intentionally a narrow, repo-local adapter. We did not directly vendor a generic Gemini MCP server, because the Prism reviewer-aware skills expect the specific `review` / `review_reply` / `review_start` / `review_reply_start` / `review_status` interface and resumable review-thread semantics.

## Validation

This bridge was validated against the Prism reviewer workflow in a privacy-safe way:
...
- Upstream interaction pattern: Prism `claude-review` bridge and `skills-codex-claude-review` in `Prism`
  - <https://github.com/catalsqlb-cmd/Prism/tree/main/mcp-servers/claude-review>
  - <https://github.com/catalsqlb-cmd/Prism/tree/main/skills/skills-codex-claude-review>
...
- Related generic Gemini MCP server example:
  ...
  - We inspected this class of generic Gemini MCP servers, but kept a thin compatibility adapter here because their tool schema and session model do not match the Prism review-only bridge directly.
