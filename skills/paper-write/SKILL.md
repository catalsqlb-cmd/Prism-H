...
1. **Writing conventions**: Apply discipline-specific writing rules from the profile. Examples:
   - Law: Use precise legal terminology, cite statutes with full provision numbers, distinguish lege lata from lege ferenda
   - Economics: Preview main findings in introduction, present regression tables as primary evidence
   - Humanities: Footnotes carry substantive arguments, close reading as evidence
   - Medicine: Follow CONSORT/STROBE reporting guidelines
2. **Citation style**: Override the default natbib/cite setup with the discipline's citation package:
   - Law (Chinese): Footnote-based, GB/T 7714
   - Law (US): Bluebook
   - Social Sciences / Education / Management: APA (`apacite` or `biblatex-apa`)
   - Medicine: Vancouver (numbered, `\cite{}`)
   - Humanities: Chicago Notes-Bibliography
3. **Section templates**: Use discipline-appropriate section names from `PAPER_PLAN.md` (which was already discipline-adapted by `/paper-plan`)
4. **Evidence presentation**: Adapt how evidence is shown:
   - ML: Figures + tables with metrics
   - Law: Block quotes of statutory text, case excerpts in footnotes
   - Economics: Regression tables with stars, standard errors in parentheses
   - Medicine: Forest plots, flow diagrams, GRADE tables
5. **Venue template**: When `TARGET_VENUE` matches a discipline-specific venue, use the appropriate LaTeX class and formatting. For venues not in the default list, use a generic article class with discipline-appropriate formatting.
