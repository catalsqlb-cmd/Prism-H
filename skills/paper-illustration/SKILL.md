...
description: "Generate publication-quality AI illustrations for academic papers via a Claude-supervised iterative refinement loop. Backends: Gemini image generation (Nano Banana Pro, default) or Codex native image generation (codex-image2 bridge, set backend=codex). Creates architecture diagrams and method illustrations. Use when user says \"生成图表\", \"画架构图\", \"AI绘图\", \"paper illustration\", \"generate diagram\", \"codex 绘图\", or needs visual figures for papers."
argument-hint: [description-or-method-file]
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, Agent, mcp__codex__codex, mcp__codex__codex-reply, mcp__codex-image2__generate, mcp__codex-image2__generate_start, mcp__codex-image2__generate_status, WebSearch
---
...
- **BACKEND = `gemini`** — Which renderer to use for Step 4. One of:
  - `gemini` (default) — Gemini image generation (Paperbanana / Nano Banana Pro). Requires `GEMINI_API_KEY`.
  - `codex` — Codex native image generation via the local `codex-image2` MCP bridge. Requires the Codex app-server + `tools/paper_illustration_image2.py` helper. See **Backend: codex** below.
  - Override with `— backend: codex`.
- **IMAGE_MODEL = `gemini-3-pro-image-preview`** — Paperbanana (Nano Banana Pro) for image rendering (gemini backend)
- **REASONING_MODEL = `gemini-3-pro-preview`** — Gemini for layout optimization and style checking (gemini backend)
...
> **The whole Claude-supervised loop below (planning, layout/style refinement, STRICT review, scoring, iteration) is backend-agnostic.** Only **Step 4 (rendering)** differs by `BACKEND`. The Steps 0–8 body documents the `gemini` path; for `codex`, use the **Backend: codex** section to replace Step 0's preflight and Step 4's render call.

## Backend: codex (codex-image2 bridge)

When `BACKEND = codex`, keep every Claude-supervised step identical except swap the renderer:

- **Step 0 (preflight)** — instead of the `GEMINI_API_KEY` check, run:
  ```bash
  python3 tools/paper_illustration_image2.py preflight \
    --workspace <cwd> --json-out figures/ai_generated/preflight.json
  ```
  Proceed only if the JSON reports `ok=true`. Accept only native `imageGeneration` output — reject any shell/Python bitmap fallback.
- **Step 4 (render)** — instead of the Gemini curl call, render through the bridge:
  - `mcp__codex-image2__generate_start` with `prompt` (the final style-verified prompt), `cwd`, `outputPath: figures/ai_generated/figure_v{N}.png`, `system: "Academic paper figure. Prefer crisp English labels."`, `timeoutSeconds: 180`.
  - Poll `mcp__codex-image2__generate_status` with bounded waits until `done=true` (`status=completed` or `failed`). On failure, surface the bridge error honestly.
- **Step 8 (finalize)** — after a figure scores ≥ 9, emit canonical artifacts via the helper instead of hand-writing them:
  ```bash
  python3 tools/paper_illustration_image2.py finalize --workspace <cwd> \
    --best-image figures/ai_generated/figure_vN.png --score 9 \
    --review-summary "Accepted after strict review; labels and arrows are paper-ready."
  python3 tools/paper_illustration_image2.py verify --workspace <cwd> \
    --json-out figures/ai_generated/verify.json
  ```
  (Repair path: if rendering succeeded but artifacts were skipped, re-run `finalize` then `verify`.)

Everything else — the CVPR/NeurIPS style guide, Step 1 planning, Steps 2–3 layout/style refinement (you may use `mcp__codex__codex` for an optional text-only critique), Step 5 STRICT review checklist, Steps 6–7 decision/refinement — applies unchanged.
