...
The research wiki is a persistent, per-project knowledge base that accumulates structured knowledge across the entire Prism research lifecycle. Unlike one-off literature surveys that are used and forgotten, the wiki **compounds** — every paper read, idea tested, experiment run, and review received makes the wiki smarter.
...
research-wiki/
  ...
  query_pack.md          # compressed summary for /research-refine (auto-generated, max 8000 chars)
  papers/
...
is the single implementation of paper ingest in Prism (per
...
Other skills (`/research-lit`, `/semantic-scholar`, `/exa-search`) call the same helper directly in
...
### Hook 2: `/research-refine` reads AND writes wiki

**Before research-question formulation:**
```
if research-wiki/query_pack.md exists (and < 7 days old):
    prepend query_pack to landscape context
    treat failed questions/theses as banlist
    treat top gaps as search seeds
    still run fresh literature search for foundational + recent works
```

**After formulation (THIS IS CRITICAL — without it, ideas/ stays empty):**
```
for question in all_candidate_questions (kept + rejected):
    /research-wiki upsert_idea(question)
    for source_id in question.based_on:
        add_edge(question.node_id, source_id, "inspired_by")
    for gap_id in question.target_gaps:
        add_edge(question.node_id, gap_id, "addresses_gap")
rebuild query_pack
log "research-refine wrote N candidate questions to wiki"
```

### Hook 3: After `/argument-stress-test` verdict
...
rebuild query_pack
log "argument-stress-test: exp_id updated, verdict=..."
```

## Re-ideation Trigger

After significant wiki updates, suggest re-running `/research-refine`:

- ≥5 new sources ingested since last formulation
- ≥3 new failed/partial questions since last formulation
