...
```json
{
  ...
  "trace_path":       ".prism/traces/proof-checker/<date>_run<NN>/",
...
false-negative rate (flawed proofs passed) stays < 0.15 and false-positive
rate (valid proofs wrongly flagged) stays < 0.10.

- Gold examples: `evals/gold/proof-checker/*.json`
- Run this skill on each example's `input`, record verdicts to
  `evals/runs/<date>/proof-checker.jsonl`, then score:
  ```bash
  python3 tools/audit_calibrate.py \
    --gold-dir evals/gold/proof-checker \
    --predictions evals/runs/<date>/proof-checker.jsonl
  ```
- Exit 1 = recalibrate (adjust rigor/flagging here, or fix the blind spot the
  breached `category` rows reveal). See `evals/README.md` for the full loop.
- When this audit misses a real flaw or false-alarms in practice, add that case
  to the gold set so it never regresses again.
