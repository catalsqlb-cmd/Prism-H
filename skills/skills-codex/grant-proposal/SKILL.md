...
- Go to develop evidence → `/research-review` → `/paper-writing` (develop & publish)
...
> ⚠️ **External review fallback**: If reviewer agents are unavailable, skip external review. Note "External review skipped — no reviewer agent available. Consider running `/research-review-llm` separately." in `GRANT_REVIEW.md`. The proposal is still usable without external review.
