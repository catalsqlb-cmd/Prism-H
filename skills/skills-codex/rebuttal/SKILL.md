...
Workflow 2:   research-review (pre-submission)
...
- **AUTO_EVIDENCE = false** — When `true`, invoke `/research-refine` (or `/statute-case-mapper`, `/comparative-law`) to develop new evidence for reviewer concerns. Never fabricate evidence.
...
**Skip entirely if `AUTO_EVIDENCE` is `false`.**

If the strategy plan identifies issues that require new evidence (sources, cases, statutes, sharper arguments):

1. Generate a mini evidence plan from the reviewer concerns
2. Invoke the appropriate evidence skill, e.g. `/research-refine "rebuttal/REBUTTAL_EVIDENCE_PLAN.md"`
3. When the evidence is developed, update `ISSUE_BOARD.md`
4. If the evidence is weak or inconclusive, switch to `narrow_concession` or `future_work_boundary`
5. Save the developed evidence to `rebuttal/REBUTTAL_EVIDENCE.md`. Never fabricate evidence.
