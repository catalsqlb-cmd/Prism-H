---
name: overleaf-sync
description: "Two-way sync between a local paper directory and an Overleaf project via the Overleaf Git bridge (Premium feature). Lets you keep Prism audit/edit workflows on the local copy while collaborators edit in the Overleaf web UI. Token never touches the agent — user does the one-time auth via macOS Keychain. Use when user says \"同步 overleaf\", \"overleaf sync\", \"推送到 overleaf\", \"connect overleaf\", \"Overleaf 桥接\", \"pull overleaf\", \"push overleaf\", or wants to bridge their Prism paper directory with an Overleaf project."
...
- **Prism** can read your changes, run audits (`/paper-claim-audit`, `/citation-audit`, `/research-refine`), and push fixes back
...
- You want to use Overleaf as the editing surface (better collaboration, shared with team) but still run Prism pipelines locally
- You want to take an existing local Prism paper and push it to Overleaf for a co-author to edit
...
│  (Prism audits)  │                          │ (git bridge)    │     (collaborators)
└─────────────────┘                          └─────────────────┘
```

The `paper-overleaf/` directory is a **git clone of the Overleaf project**. The `paper/` directory is the working copy where Prism skills run. They are kept in sync via `rsync`.
...
Run this in your own terminal (NOT through me):
    ...
    bash <PRISM_REPO>/tools/overleaf_setup.sh <project-id-or-url>
...
bash <PRISM_REPO>/tools/overleaf_audit.sh .   # must report "Audit clean"
```
...
Use after Prism skills have edited `paper/` and you want collaborators on Overleaf to see the changes.
...
# 4. Commit + push
git add -A
git commit -m "<descriptive message — what Prism changed and why>"
git push
```

**Commit message protocol**: include the Prism skill that produced the change so collaborators on Overleaf understand provenance. Examples:
...
| No  | Yes | Local Prism edits unsynced | Run `push` |
...
3. Show the user `git log origin/master ^HEAD` (their Overleaf commits) and `git log HEAD ^origin/master` (local Prism commits).
...
- If the user is in an active Overleaf editing session, Prism skills should **read-only** access `paper/` until the user runs `/overleaf-sync pull`.
- If Prism is in the middle of `/research-refine` or `/paper-write`, the user should pause Overleaf editing until the loop finishes and `/overleaf-sync push` is run.
...
- `paper/` directory unchanged in role — still the Prism working copy
