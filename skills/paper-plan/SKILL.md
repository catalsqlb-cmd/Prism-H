...
1. **Paper structure**: Replace the default empirical/theory/method templates (Step 2) with the discipline's paper structure from the profile. Examples:
   - **Law**: Introduction → Doctrinal Framework → Normative Analysis → Comparative Law → Case Analysis → Reform Proposal → Conclusion
   - **Economics**: Introduction → Institutional Background → Data → Empirical Strategy → Results → Robustness → Conclusion
   - **Medicine**: Introduction → Methods → Results → Discussion (IMRAD)
   - **Social Sciences (qualitative)**: Introduction → Literature Review → Methodology → Findings → Discussion → Conclusion
2. **Page budgets**: Use the discipline's section-level page allocations instead of ML defaults.
3. **Citation style**: Set the citation package and format per discipline (Bluebook for law, APA for social sciences, Vancouver for medicine, natbib for ML, IEEE numeric for EE).
4. **Claims-Evidence Matrix**: Adapt evidence types — for law: "statutory provision + judicial decision + comparative example"; for economics: "regression table + robustness check + placebo test"; for medicine: "primary outcome + forest plot + GRADE assessment".
5. **Paper type classification**: Use discipline-appropriate categories:
   - Law: doctrinal analysis / law-and-economics / comparative law / empirical legal study
   - Economics: empirical / theoretical / structural estimation
   - Social Sciences: quantitative / qualitative / mixed methods
   - Humanities: interpretive / historical / critical
6. **Venue formatting**: Load venue-specific rules for the target venue. When `TARGET_VENUE` matches a discipline-specific venue (e.g., `法学研究`, `AER`, `NEJM`), load the corresponding formatting requirements from the discipline profile.
...
**First check for `CLAIMS_FROM_RESULTS.md`** — if it exists (generated by `/argument-stress-test` at the end of Workflow 2), use it as the starting point for claims. This file contains validated claims already mapped to experiment evidence. Merge with any additional claims from the narrative documents below.
