...
description: Search published venue papers (journals, university presses, conference proceedings) via Semantic Scholar API. Provides citation counts, venue metadata, and TLDR. Use when user says "search semantic scholar", "find journal papers", "venue papers", "citation search", or wants published scholarly literature.
...
This skill targets **published venue** scholarship:

| Skill | Source | Best for |
|-------|--------|----------|
| `/research-lit` (web) | WebSearch / arXiv | Broad surveys, preprints, foundational works |
| `/semantic-scholar` | Semantic Scholar API | **Published** journal/conference papers and books with citation counts, venue info, TLDR |
...
- If present → paper is also on arXiv. Note this in output but do NOT re-fetch the preprint.
- If absent → paper is **venue-only** (e.g. a journal article or book without a preprint). This is the unique value of this skill.
...
```text
/research-lit "topic"    - multi-source review: Zotero + local PDFs + web + S2
...
- If the S2 API is unreachable, suggest using `/research-lit "topic" - sources: web` as fallback.
