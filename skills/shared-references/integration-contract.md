...
When one Prism skill delegates work to another (or to persistent project
state), the coupling must be **engineered**, not assumed. This document
formalizes what every cross-skill integration inside Prism must provide.
...
2. **Research wiki ingest no-op (2026-04-21).** `/research-wiki init`
   ...
   `/semantic-scholar`, `/exa-search`,
   raw `Read`/`WebFetch` — none carried a wiki-ingest hook, and the one
   that did (`/research-lit`) only had soft prose
...
Every integration between two Prism skills (or between a skill and a
...
invocations (one-off skills like `/semantic-scholar 2501.12345`).
...
- ⚠️ `/research-wiki sync` that scans `.prism/traces/` for arxiv IDs —
...
## Known Prism integrations under this contract

| Integration | Predicate | Helper | Artifact | Checklist | Backfill | Verifier |
|---|---|---|---|---|---|---|
| Submission audits (`max`/`beast`) | `paper/.prism/assurance.txt = submission` | `verify_paper_audits.sh` + 3 audit skills emit JSON | `paper/PROOF_AUDIT.json`, `PAPER_CLAIM_AUDIT.json`, `CITATION_AUDIT.json` + `paper/.prism/audit-verifier-report.json` | Phase 6.0 pre-flight checklist | Rerun the failed audit | `verify_paper_audits.sh` (exit 1 blocks) |
...
| paper-illustration codex backend finalization | `tools/paper_illustration_image2.py preflight --workspace <cwd>` returns `ok=true` | `paper_illustration_image2.py` (`preflight`, `finalize`, `verify`) | `figures/ai_generated/figure_final.png`, `latex_include.tex`, `review_log.json` | "Backend: codex" section in `paper-illustration` | `paper_illustration_image2.py finalize --workspace <cwd> --best-image <png>` | `paper_illustration_image2.py verify` (diagnostic, exit 1 on missing artifacts) |
| LLM Wiki scaffold + maintain | `<root>/CLAUDE.md` + `<root>/wiki/index.md` exist (a valid wiki root) | `llm_wiki.py` (`init`, `lint`, `log`) + `query_wiki.py` (search) | `<root>/wiki/index.md`, `wiki/log.md` append, `wiki/**/*.md` pages | Subcommands section in `llm-wiki` | `llm_wiki.py init <root>` (idempotent; re-run after manual edits + `sync`) | `llm_wiki.py lint <root>` (exit 1 on missing-page refs; warnings diagnostic) |
| Audit self-calibration | `evals/gold/<skill>/` exists with labeled examples | `audit_calibrate.py` (scores verdicts vs gold) | `evals/runs/<date>/<skill>.report.json` (FNR/FPR + confusion matrix) | Calibration loop in `evals/README.md` + "Self-calibration" section in each audit skill | Re-run the audit skill on `evals/gold/<skill>/` → new predictions jsonl | `audit_calibrate.py` (exit 1 when FNR>0.15 or FPR>0.10; dev-time gate, not a per-paper gate) |
