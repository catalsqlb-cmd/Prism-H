...
```
.prism/traces/<skill-name>/<YYYY-MM-DD>_run<NN>/
...
- `<skill-name>`: the Prism skill that triggered this call (e.g., `research-review`)
...
After writing a trace, append a compact summary event to `.prism/meta/events.jsonl`:

```json
{"event":"review_trace","skill":"research-review","purpose":"round-1-review","thread_id":"...","trace_path":".prism/traces/research-review/2026-04-15_run01/","status":"ok"}
```

This allows downstream review tooling to discover traces without reading the full trace files.

## Privacy

- `.prism/traces/` should be in `.gitignore` — traces are project-local, never committed
