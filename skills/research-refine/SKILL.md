...
User input (PROBLEM + vague APPROACH)
  ...
  -> Optional handoff: develop the evidence base (sources / cases / statutes / argument reconstruction)
```
...
- **MAX_CORE_EVIDENCE_BLOCKS = 3** — Default cap for core validation blocks (key sources, cases, arguments) inside this skill.
...
## Discipline-Aware Mode

At startup, check for `PRISM_PROFILE.md` in the project root. If found, load it and adapt behavior:

1. **Review dimensions**: Replace the default 7 ML dimensions (Phase 2 reviewer prompt) with the discipline's review dimensions and weights from the profile.
2. **Reviewer persona**: Change the reviewer prompt from "senior ML reviewer for a top venue (NeurIPS/ICML/ICLR)" to the appropriate discipline equivalent (e.g., "senior legal scholar reviewing for 法学研究" or "senior economist reviewing for AER").
3. **Methodology principles**: The four core principles still apply, but adapt their expression:
   - "Smallest adequate mechanism" → for law: "smallest adequate doctrinal intervention"; for economics: "most parsimonious identification strategy"
   - "Frontier leverage" → for law: "interdisciplinary methods (economic analysis, empirical methods) when they naturally strengthen the argument"; for economics: "modern causal inference tools when appropriate"
4. **Validation design** (Step 1.5): Design validation appropriate to the discipline — case analysis for law, robustness checks for economics, thick description for qualitative social science, etc.
5. **Proposal template** (Step 1.6): Adapt section names to match the discipline's conventions (e.g., replace "System Overview" with "Analytical Framework" for law; replace "Training Plan" with "Empirical Strategy" for economics).

If `PRISM_PROFILE.md` does not exist, fall back to the default ML-oriented behavior (fully backward compatible).

> See `shared-references/prism-routing.md` for the full routing protocol and `shared-references/prism-profiles/` for available profiles.
...
- Ensure one evidence block directly supports the **Problem Anchor**.
- If complexity risk exists, include one **simplification or deletion check**.
- If a single source/framework is central, include one **necessity check** showing why that choice matters.
- Default to **1-3 core evidence blocks** and leave the full development roadmap to the evidence-development stage.
...
## Proposed Argument / Framework
### Complexity Budget
- Reused / inherited framework or doctrine:
- New conceptual moves introduced:
- Tempting additions intentionally not used:

### Structure Overview
[Step-by-step argument structure or outline]

### Core Move
- Premises / inputs:
- Inferential or doctrinal structure:
- What does the work (key distinction, mechanism, or interpretive claim):
- Why this is the main novelty:

### Optional Supporting Move
- Only include if truly necessary:
- Premises / inputs:
- How it supports the core claim:
- Why it does not create contribution sprawl:

### Framework / Method Usage
- Which theoretical framework, doctrine, or method is used:
- Exact role in the argument:
- Why it is more natural than an alternative framework:

### Integration into the Broader Argument
[Where the new move attaches, what is assumed, what is contested, order of exposition]

### Development Plan
[Sequence of sections, sources/cases to develop, comparative material, interpretive steps]

### Failure Modes and Objections
- [Objection / weak point]:
- [How to detect]:
- [Response or mitigation]:

### Novelty and Elegance Argument
[Closest work, exact difference, why this is a focused contribution rather than a survey or a pile-up of claims]

## Claim-Driven Validation Sketch
### Claim 1: [Main claim]
- Minimal evidence:
- Counter-positions / objections to address:
- Standard of support:
- Expected evidence:
...
## Evidence Handoff Inputs
- Must-prove claims:
- Must-address objections:
- Critical sources / cases / texts:
- Highest-risk assumptions:

## Effort & Timeline Estimate
- Estimated research/reading effort:
- Source-gathering / fieldwork cost:
- Timeline:
```
...
## Next Steps
- If READY: proceed to develop the evidence base (`/statute-case-mapper`, `/comparative-law`, `/argument-stress-test`, or source/corpus work)
- If REVISE: manually address the remaining argument weaknesses, then re-run `/research-refine`
- If RETHINK: revisit the core thesis, possibly re-surveying with `/research-lit`
```
...
Final proposal: refine-logs/FINAL_PROPOSAL.md
Suggested next step: develop the evidence base
```
...
- **Frameworks are a prior, not a decoration.** Adopt a theoretical framework or method when it sharpens the argument, not when it only makes the proposal sound sophisticated.
- **Minimal evidence.** Inside this skill, the evidence blocks only need to prove the core claims.
- **Review the argument, not the citation count.** A long source list is not novelty.
...
- **Do not fabricate evidence.** Only describe expected evidence and planned source/case/argument work.
- **Be specific about source and method assumptions.** Vague "we'll cite some cases" is not enough.
- **Document everything.** Save every raw review, every anchor check, every simplicity check, and every major method change.

## Composing with Other Skills

This skill sits between literature review and evidence development:

```
/research-refine-pipeline              -> one-shot refine + evidence planning
/research-lit "direction"       -> survey the literature
/research-refine "PROBLEM: ... | THESIS: ..."  <- you are here
develop evidence                -> statute-case-mapper / comparative-law / argument-stress-test / source work
/research-review               -> iterate on the draft and argument
```

Typical flow:

1. `/research-lit` or local reading gives you a problem and a vague thesis direction
2. `/research-refine` turns that into an anchored, elegant, well-grounded thesis plan
3. the evidence-development stage turns the final proposal into a detailed claim-driven evidence roadmap
4. `/research-refine-pipeline` is the one-shot wrapper when the user wants both stages in a single request
5. evidence development builds the sources, cases, and arguments
6. Later loops operate on the developed evidence, not just the thesis

This skill also works standalone if you already know the problem and just need the thesis to become concrete.
