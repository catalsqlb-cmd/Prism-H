---
name: citation-audit
description: "Zero-context verification that every bibliographic entry in the paper is real, correctly attributed, and used in a context the cited paper actually supports. Uses a fresh cross-model reviewer with web/DBLP/arXiv lookup to catch hallucinated authors, wrong years, fabricated venues, version mismatches, and wrong-context citations (cite present but the cited paper does not establish the claim). Optional L3 claim-level mode decomposes each citing sentence into atomic claims and verifies each against a locator-anchored excerpt fetched from the actual source. Use when user says \"审查引用\", \"check citations\", \"citation audit\", \"verify references\", \"引用核对\", \"claim 级引用审计\", or before submission to ensure bibliography integrity."
argument-hint: "[paper-directory-or-bib-file] [— level: L2|L3]"
...
3. **Context appropriateness** — the cited paper actually supports the claim it is being used to support in the manuscript. This layer has two depths:
   - **L2 (default)** — the reviewer judges support from the citing sentence plus web/DBLP/arXiv lookup. Fast, catches most wrong-context cites.
   - **L3 (claim-level, opt-in via `— level: L3`)** — decompose the citing sentence into atomic claims, fetch the **actual source at a locator anchor** (page/section/figure/equation/quote), and verify each claim against a **verbatim excerpt**. Catches the subtle cases L2 misses: a real and topically-relevant source whose specific cited locator does not contain the claimed number/result.

This skill is the fourth layer of Prism's evidence-and-claim assurance, complementing `paper-claim-audit` (code), `argument-stress-test` (science verdict), and `paper-claim-audit` (numerical claims). Together they form a bottom-up integrity stack from raw evaluation code to manuscript bibliography.
...
Save the extracted contexts to `paper/.prism/citation-audit/contexts.txt` so the reviewer can read it directly. Use the paper-dir-relative path `.prism/citation-audit/contexts.txt` when recording the file in `audited_input_hashes`; do not stage under `/tmp` or other transient locations that the verifier cannot rehash later.
...
Save the response to `.prism/traces/citation-audit/<date>_runNN/<key>.md` per the review-tracing protocol.

### Step 3.5: L3 — claim-level verification (locator-anchored)

Run this step only when invoked with `— level: L3`, or automatically for the
**load-bearing** citations: those that attribute a specific number, theorem,
SOTA claim, or causal finding to the cited work (the cites whose failure would
materially mislead a reader). L2 stays the default for the long tail of
framing/related-work cites.

L3 turns "does this source roughly support the sentence?" into "does the
**specific locator** in this source contain the **specific claim** being
attributed to it?" — the failure mode L2 misses (real, on-topic paper; wrong
page/table/number).

For each selected `(key, use)`:

1. **Decompose** the citing sentence into atomic claims. Each claim is the
   smallest assertion the sentence attributes to the cited work, with an id:
   ```
   c1: "training compute grew 10x per year since 2018"
   c2: "this trend is specific to the deep-learning era"
   ```
   Strip claims the sentence does not actually attribute to *this* cite.

2. **Locate** the anchor. From the sentence (and any locator already in the
   `\cite` / nearby text — page, section, table, figure, equation), determine
   where in the source the support should live. If no explicit locator, target
   the most specific structural element that would carry the claim (abstract
   for a headline result, a named table for a number, a theorem for a bound).

3. **Fetch** the actual source at the anchor. Prefer in order: arXiv HTML
   (`https://arxiv.org/abs/<id>` → ar5iv/HTML), publisher DOI landing, the PDF
   text, OpenReview. Use `WebFetch` (or `/semantic-scholar`). Capture
   a **verbatim excerpt** (1–4 sentences, or the relevant table row) that
   contains — or demonstrably lacks — the claimed content. Never paraphrase the
   source in place of quoting it; the excerpt is the evidence anchor.

4. **Judge each claim** against the fetched excerpt:
   - `SUPPORTED` — the excerpt states the claim (or entails it directly).
   - `PARTIAL` — the excerpt supports a weaker version; the sentence overstates.
   - `UNSUPPORTED` — the excerpt does not contain the claim (wrong locator, or
     the source simply does not establish it).
   - `CONTRADICTED` — the excerpt asserts the opposite.
   Record the verbatim excerpt and its URL with every verdict. If the source
   cannot be fetched, mark the claim `UNVERIFIABLE` (not SUPPORTED) and say why.

5. **Roll up** the use-level verdict from its claims:
   - any load-bearing claim `UNSUPPORTED`/`CONTRADICTED` → use verdict `WRONG`
   - any `PARTIAL` (and none worse) → `WEAK`
   - all `SUPPORTED` → `SUPPORTS`
   - only `UNVERIFIABLE` claims → `UNCERTAIN` (do not silently pass)

Save the per-claim evidence to the same trace file. L3 is wall-clock expensive
(a real fetch per load-bearing cite) — scope it to the cites that matter.
...
When run at **L3**, each `use` additionally carries the claim-level evidence
(`level: "L3"` plus a `claims` array); L2 uses omit `claims`:

```json
{
  "file": "sections/1.intro.tex", "line": 11, "verdict": "WRONG", "level": "L3",
  "claims": [
    {
      "claim_id": "c1",
      "claim_text": "training compute grew 10x per year since 2018",
      "locator": "Table 2",
      "support": "UNSUPPORTED",
      "source_excerpt": "Table 2 reports a ~6-month doubling time for the Deep Learning era (≈4x/year).",
      "anchor_url": "https://arxiv.org/abs/2202.05924"
    }
  ]
}
```

`support` is one of `SUPPORTED | PARTIAL | UNSUPPORTED | CONTRADICTED | UNVERIFIABLE`.
...
- **L3 is anchored, not vibes** — at `— level: L3`, every claim verdict must carry a verbatim source excerpt + URL. A claim with no fetched excerpt is `UNVERIFIABLE`, never `SUPPORTED`. Scope L3 to load-bearing cites (numbers, theorems, SOTA/causal claims); leave framing cites at L2.

## Comparison with Other Audit Skills

| Skill | What it audits | What it catches |
|-------|---------------|-----------------|
| `/paper-claim-audit` | Evaluation code | Fake ground truth, self-normalized scores, phantom results |
| `/argument-stress-test` | Result-to-claim mapping | Claims unsupported by evidence |
...
After each `mcp__codex__codex` reviewer call, save the trace following `shared-references/review-tracing.md`. Use `tools/save_trace.sh` or write files directly to `.prism/traces/citation-audit/<date>_run<NN>/`. Respect the `--- trace:` parameter (default: `full`).
...
- `.prism/traces/citation-audit/<date>_runNN/` (per-entry review traces)
...
```json
{
  ...
  "trace_path":       ".prism/traces/citation-audit/<date>_run<NN>/",
...
`paper/.prism/` so the verifier can rehash reproducibly. Do NOT hash
...
- `/paper-claim-audit` — sibling skill for evaluation code integrity
- `/argument-stress-test` — claim verdict assignment from results
...
This detector is regression-tested against a labeled gold set so its
false-negative rate (missed bad citations) stays < 0.15 and false-positive
rate (clean citations wrongly flagged) stays < 0.10.

- Gold examples: `evals/gold/citation-audit/*.json`
- Run this skill on each example's `input`, record verdicts to
  `evals/runs/<date>/citation-audit.jsonl`, then score:
  ```bash
  python3 tools/audit_calibrate.py \
    --gold-dir evals/gold/citation-audit \
    --predictions evals/runs/<date>/citation-audit.jsonl
  ```
- Exit 1 = recalibrate (tighten/loosen flagging here, or fix the blind spot the
  breached `category` rows reveal). See `evals/README.md` for the full loop.
- When this audit misses a real problem or false-alarms in practice, add that
  case to the gold set so it never regresses again.
