...
```text
Stage 1: research-lit + research-question formulation
Stage 2: develop evidence (sources / cases / arguments)
Stage 3: research-review (pre-submission)
Stage 4: paper-writing
Stage 5: rebuttal (post-submission external reviews)
```
...
- **AUTO_EVIDENCE = false** — When `true`, automatically invoke `/research-refine` (or `/statute-case-mapper`, `/comparative-law` for legal work) to develop supplementary evidence when the strategy plan identifies reviewer concerns that require new sources, cases, or analysis. When `false` (default), pause and present the evidence gap to the user for manual handling. Never fabricate evidence.
...
### Phase 3.5: Evidence Sprint (when AUTO_EVIDENCE = true)

**Skip entirely if `AUTO_EVIDENCE` is `false` — instead, pause and present the evidence gaps to the user.**

If the strategy plan identifies issues that require new evidence (tagged `response_mode: grounded_evidence` with `evidence_source: needs_development`):

1. Generate a mini evidence plan from the reviewer concerns:
   - What to develop (additional sources, cases, statutes, a sharper argument reconstruction, a comparative case)
   - Success criterion (what would satisfy the reviewer)
   - Which skill to use (`/research-refine`, `/statute-case-mapper`, `/comparative-law`, `/argument-stress-test`)

2. Invoke the appropriate evidence skill with the mini plan, e.g.:
   ```
   /research-refine "rebuttal/REBUTTAL_EVIDENCE_PLAN.md"
   ```

3. When the evidence is developed, update `ISSUE_BOARD.md`:
   - Tag completed items as `user_confirmed_evidence`
   - Update evidence source for relevant issue cards

4. If the evidence is weak or inconclusive:
...
**Time guard**: If the estimated time to develop the evidence exceeds the rebuttal deadline, skip and flag for manual handling.
...
After each `mcp__codex__codex` or `mcp__codex__codex-reply` reviewer call, save the trace following `shared-references/review-tracing.md`. Use `tools/save_trace.sh` or write files directly to `.prism/traces/<skill>/<date>_run<NN>/`. Respect the `--- trace:` parameter (default: `full`).
