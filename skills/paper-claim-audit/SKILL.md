...
|-------|-------------------|
| `/paper-claim-audit` | Is the experiment code honest? (fake GT, normalization fraud) |
| `/argument-stress-test` | Does the data scientifically support this claim? |
...
2. **After `/research-refine`** — recheck if improvement loop changed numbers
3. **Before submission** — final verification

## Integration with Other Skills

### Read by `/research-refine` (if exists)
...
Same pattern as `/paper-claim-audit`:
...
```json
{
  ...
  "trace_path":       ".prism/traces/paper-claim-audit/<date>_run<NN>/",
...
false-negative rate (unsupported/overgeneralized claims missed) stays < 0.15
and false-positive rate (well-supported claims wrongly flagged) stays < 0.10.

- Gold examples: `evals/gold/paper-claim-audit/*.json`
- Run this skill on each example's `input`, record verdicts to
  `evals/runs/<date>/paper-claim-audit.jsonl`, then score:
  ```bash
  python3 tools/audit_calibrate.py \
    --gold-dir evals/gold/paper-claim-audit \
    --predictions evals/runs/<date>/paper-claim-audit.jsonl
  ```
- Exit 1 = recalibrate (adjust flagging criteria here, or fix the blind spot the
