...
**This is a parallel branch, not part of the linear literature→question→evidence→review pipeline.** After `/research-lit` and research-question formulation produce a validated question, the user can either:
- Go to develop the evidence → `/research-review` → `/paper-writing` (develop & publish)
- Go to `/grant-proposal` (write funding application first, then develop after funding)

```
                    ┌→ develop evidence → /research-review → /paper-writing  (publish track)
research question ──┤
                    └→ /grant-proposal → [get funded] → develop evidence → ...  (funding track)
```
...
1. Read `lit-stage/LIT_REVIEW.md` if it exists (from `/research-lit`); fall back to `./LIT_REVIEW.md` if not found
2. Read `refine-logs/FINAL_PROPOSAL.md` if it exists (from `/research-refine`)
3. Read `RESEARCH_LOG.md` if it exists (evidence-development notes)
4. Read `review-stage/AUTO_REVIEW.md` if it exists (from `/research-review` — prior review feedback is gold for grants); fall back to `./AUTO_REVIEW.md` if not found
...
- No research question at all → suggest running `/research-lit` and formulating a research question first
...
> ⚠️ **Codex MCP fallback**: If `mcp__codex__codex` is not available (no OpenAI API key), skip external review. Note "External review skipped — no Codex MCP available. Consider running `/research-review — backend: llm` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.
...
/research-refine "question"              ← sharpen the research question
/grant-proposal "question — NSSFC"       ← this skill: write the grant proposal
                                          ← [submit & get funded]
develop evidence                         ← build the evidence base with funding
/research-review "draft"                ← iterate until submission-ready
/paper-writing                           ← write the paper
```

### Publish Track (skip this skill)

```
/research-lit → develop evidence → /research-review → /paper-writing → submit
```
