...
1. **Source priority**: Replace the default source order with the discipline's `Literature Sources` table. For example:
   - Law → CNKI, PKULaw/Westlaw, HeinOnline, SSRN, Google Scholar
   - Economics → NBER, SSRN, EconLit, Google Scholar, CNKI
   - Medicine → PubMed, Cochrane, Web of Science, Embase, CNKI
   - Communications → IEEE Xplore, arXiv, Semantic Scholar, ACM DL
2. **Venue filtering**: When ranking papers, prioritize those from the discipline's Tier A venues.
3. **Time window**: Humanities and law may need wider time windows (foundational works from decades ago are still highly relevant). ML focuses on last 2-3 years.
4. **Search terminology**: Adapt search queries to use discipline-appropriate terminology and databases.
5. **Output format**: For law, include jurisdiction and court level in the paper table. For medicine, include study design (RCT, cohort, etc.) and evidence level.

If `PRISM_PROFILE.md` does not exist, fall back to the default behavior below (fully backward compatible).

> See `shared-references/prism-routing.md` for the full routing protocol.
...
| 5 | **Semantic Scholar API** | `semantic-scholar` | `tools/semantic_scholar_fetch.py` exists | Published venue papers (journals, university presses) with structured metadata: citation counts, venue info, TLDR. **Only runs when explicitly requested** via `— sources: semantic-scholar` or `— sources: web, semantic-scholar` |
| 6 | **Exa Search** | `exa` | `tools/exa_search.py` and installed `exa-py` SDK | AI-powered broad web search with content extraction (highlights, text, summaries). Covers blogs, docs, news, institutions, and research papers. **Only runs when explicitly requested** via `— sources: exa` or `— sources: all, exa` |
...
# If not found, check Prism install
...
unavailable (e.g., offline on a non-Prism machine), log the gap and let
